      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm p-6 mb-8">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-1">{t('buddies.destination')} *</label>
              <select
                value={formDest}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">{t('buddies.dateFrom')}</label>
                <input
                  type="text"
                  value={formDateFrom}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1">{t('buddies.dateTo')}</label>
                <input
                  type="text"
                  value={formDateTo}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-1">{t('buddies.intro')} *</label>
              <textarea
                value={formIntro}
            ...
            <div>
              <label className="block text-sm font-medium text-slate-600 mb-1">{t('buddies.yourName')}</label>
              <input
                type="text"
                value={formAuthor}
                onChange={(e) => setFormAuthor(e.target.value)}
                placeholder={t('buddies.anonymous')}
                className="w-full px-3 py-2 rounded-lg border border-slate-200 focus:ring-2 focus:ring-amber-400 max-w-xs"
                maxLength={30}
              />
            </div>
            ...
            filtered.map((p) => (
              ...
              >
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <span className="px-2 py-0.5 rounded bg-teal-100 text-teal-700 text-sm font-medium">{p.destination}</span>
                  {(p.dateFrom || p.dateTo) && (
